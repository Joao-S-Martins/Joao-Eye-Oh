...
const GalleryImage = forwardRef<HTMLElement, GalleryImageProps>((props, ref) => {
  ...
  const [url, setUrl] = useState(
    // HACK (joao) Figure out why react isn't updating with new URL
    isSet ? src.src.images[src.src.images.length - 1].path : src);
  ...
  const onClick = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) =>
    props.onClick ? props.onClick(event) : showFullImage(url);
  ...
  return (
    <figure
      ...
      {isVid ?
        <video
          autoPlay
          className={styles[`vid${className}`]}
          loop
          muted
          preload="true"
          src={src} /> :
